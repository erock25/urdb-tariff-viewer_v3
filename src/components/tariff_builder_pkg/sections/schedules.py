...
def _render_simple_schedule_editor(data: Dict, num_periods: int, rate_type: str) -> None:
    """
    ...
    st.markdown(f"#### {'Weekday' if rate_type == 'energy' else 'Demand Weekday'} Schedule")
    ...
    with st.form(form_key, clear_on_submit=False):
        ...
        with col1:
            ...
            for hour in range(24):
                current_value = data[weekday_key][0][hour] if hour < len(data[weekday_key][0]) else 0
                period = st.selectbox(
                    f"Hour {hour}:00",
        ...
        if not weekend_same:
            ...
            for hour in range(24):
                with cols[hour % 4]:
                    current_value = data[weekend_key][0][hour] if hour < len(data[weekend_key][0]) else 0
                    period = st.selectbox(
                        f"Hr {hour}",
                        options=list(range(num_periods)),
...
def _render_advanced_schedule_editor(data: Dict, num_periods: int, rate_type: str) -> None:
    """
    ...
    if same_schedule:
        ...
        schedule_type = st.radio("Schedule to edit:", ["Weekday", "Weekend"], 
                                  horizontal=True, key=f"{rate_type}_schedule_type")
        schedule_type_lower = schedule_type.lower()
    
    # Three-step process
    st.markdown("---")
    st.markdown("### Step 1: Manage Templates")
    tip_text = ("💡 **Tip**: Create a template for each unique schedule (e.g., Summer, Winter, Shoulder)." +
                ("" if same_schedule else " **Remember to do this for both Weekdays and Weekends.**"))
    st.info(tip_text)
...
def _initialize_default_templates(data: Dict, schedule_key: str, num_periods: int) -> Dict:
    ...
    return {
        'Template 1': {
            'name': 'Template 1',
            'schedule': data[schedule_key][0].copy() if data.get(schedule_key) else [0] * 24,
            'assigned_months': [0]
        }
    }
...
def _render_template_editor(schedule_type: str, rate_type: str, num_periods: int, data: Dict) -> None:
    ...
    with st.form(f"template_editor_{rate_type}_{schedule_type}_{selected_template}"):
        ...
        for hour in range(24):
            with cols[hour % 6]:
                current_idx = template['schedule'][hour] if hour < len(template['schedule']) else 0
                period = st.selectbox(
                    f"{hour}:00",
                    options=list(range(num_periods)),
                    format_func=lambda x: f"{labels[x]}" if x < len(labels) else f"P{x}",
                    index=current_idx,
    ...
    try:
        st.dataframe(
            schedule_df.style.background_gradient(cmap='RdYlGn_r', axis=None)
                .format("{:.4f}" if rate_structure else "{:.0f}"),
            use_container_width=True
        )
    except ImportError:
