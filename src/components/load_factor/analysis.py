...
def render_load_factor_analysis_tool(tariff_viewer: TariffViewer, options: Dict[str, Any]) -> None:
    """
    ...
    (energy_percentages, total_percentage, active_periods_list, 
     period_hour_percentages, energy_period_month_counts) = render_energy_distribution_inputs(
        tariff_data, analysis_period, selected_month
    )
    ...
    if active_periods_list and (has_tou_demand or has_flat_demand):
        ...
        st.info("ℹ️ This tariff does not have the required rate structure data for utilization analysis. "
...
def _render_calculate_button(
    ...
    if st.button("🧮 Calculate Effective Rates", type="primary", key="calc_load_factor"):
        ...
        max_valid_lf = calculate_max_valid_load_factor(energy_percentages, period_hour_percentages)
        ...
        display_load_factor_results(
            ...
            demand_inputs=demand_inputs,
            energy_percentages=energy_percentages,
            selected_month=selected_month if analysis_period == "Single Month" else None,
            has_tou_demand=has_tou_demand,
            has_flat_demand=has_flat_demand,
            analysis_period=analysis_period,
            demand_period_month_counts=demand_period_month_counts,
