...
from src.config.constants import MONTHS_FULL
...
def check_form_needs_initialization(
    ...
    return (
        labels_key not in st.session_state or
        len(st.session_state.get(labels_key, [])) != num_periods or
        len(st.session_state.get(rates_key, [])) != num_periods
    )
...
def render_rate_editing_form(
    ...
    with st.form(config.form_key):
        ...
        for i, rate_struct in enumerate(rate_structure):
            if rate_struct:
                ...
                current_label = (
                    ...
                    else (rate_labels[i] if rate_labels and i < len(rate_labels)
        ...
        if st.form_submit_button("✅ Apply Changes", type="primary"):
            ...
            st.success(f"✅ {config.rate_type.replace('_', ' ').title()} rate changes applied!")
            st.rerun()
...
def render_flat_demand_editing_form(
    ...
    if rates_key not in st.session_state or len(st.session_state.get(rates_key, [])) != 12:
        st.session_state[rates_key] = []
        st.session_state[adjs_key] = []
