"""
Bill Calculator - Core utility bill calculation engine.

This module contains the core utility bill calculation logic and validation functions.
It is framework-agnostic and has no Streamlit dependencies.
...
def validate_tariff(tariff: Dict, default_voltage: float = 480.0) -> None:
    ...
    if (
        len(tariff["energyweekdayschedule"]) != 12
        or len(tariff["energyweekendschedule"]) != 12
    ):
    ...
    if "demandratestructure" in tariff and tariff["demandratestructure"]:
        if (
            "demandweekdayschedule" not in tariff
            or "demandweekendschedule" not in tariff
        ):
            raise InvalidTariffError(
                "Demand schedules required when demand rates exist"
            )
        if (
            len(tariff["demandweekdayschedule"]) != 12
            or len(tariff["demandweekendschedule"]) != 12
        ):
    ...
    if has_demand:
        ...
        if missing_demand:
            raise InvalidTariffError(
                f"Missing demand fields: {', '.join(missing_demand)}"
            )
    ...
    if has_demand and not tariff["demandratestructure"]:
        raise InvalidTariffError(
            "Demand rate structure cannot be empty when demand charges are present"
        )
    ...
    if min_voltage > default_voltage:
        print(
            f"WARNING: Tariff minimum voltage ({min_voltage}V) is higher than default voltage ({default_voltage}V)"
        )
    if max_voltage < default_voltage:
        print(
            f"WARNING: Tariff maximum voltage ({max_voltage}V) is lower than default voltage ({default_voltage}V)"
        )
...
def get_rate_for_demand(
    structure: List[Dict],
    demand: float,
    reactive_power_charge: float = 0.0,
...
def extract_adjustments(tariff: Dict) -> Dict[str, float]:
    ...
    if "electric vehicle discount" in description.lower():
        ...
        if ev_matches:
            adjustments["ev_discount"] = (
                -float(ev_matches[0]) / 100
...
def ensure_integer_columns(
    df: pd.DataFrame, integer_columns: List[str]
) -> pd.DataFrame:
...
def vectorized_energy_charges(
    energy_periods: np.ndarray, kwh_values: np.ndarray, rate_structure: List[List[Dict]]
) -> tuple:
    """
    ...
    if is_single_tier:
        ...
        rates = np.array(
            [struct[0].get("rate", 0) if struct else 0 for struct in rate_structure]
        )
        adjs = np.array(
            [struct[0].get("adj", 0) if struct else 0 for struct in rate_structure]
        )
    ...
    try:
        ...
        with open(path, "r", encoding="utf-8") as f:
            try:
                ...
                raise InvalidTariffError(
                    f"Invalid JSON format in tariff file: {str(e)}"
                )
        ...
        if "items" in data:
            if not isinstance(data["items"], list) or len(data["items"]) == 0:
                raise InvalidTariffError(
                    "Tariff file 'items' must be a non-empty array"
                )
...
def calculate_monthly_bill(
    ...
    if tariff_data is not None:
        tariff = (
            extract_tariff_data(tariff_data) if "items" in tariff_data else tariff_data
        )
    ...
    period_mapping = {
        ...
        "demand": {0: "off_peak", 1: "mid_peak", 2: "high_peak", 3: "critical_peak"},
        "season": {
    ...
    has_demand = (
        "demandratestructure" in tariff
        and tariff["demandratestructure"]
        and "demandweekdayschedule" in tariff
        and "demandweekendschedule" in tariff
    )
    ...
    if has_demand_charges:
        ...
        demand_peaks = (
            df.groupby(["year", "month", "demand_period"])["load_kW"]
            .max()
            .reset_index()
        )
        ...
        for _, row in demand_peaks.iterrows():
            ...
            power_factor = tariff.get(
                "powerfactor", 0.95
            )  # Default power factor if not specified
    ...
    for (year, month), group in df.groupby(["year", "month"]):
        ...
        fixed_charge = tariff.get(
            "fixedchargefirstmeter", max(fixed_monthly, min_monthly)
        )
        ...
        kw_adjustments = sum(v for k, v in adjustments.items() if k.endswith("_per_kw"))
        total_kwh_adj = (
            sum(v for k, v in adjustments.items() if k.lower().endswith("kwh"))
            * total_kwh
        )
        total_kw_adj = (
            sum(v for k, v in adjustments.items() if k.lower().endswith("kw")) * peak_kw
        )
    ...
    kwh_periods = pd.json_normalize(summary_df["kwh_by_period"]).add_prefix(
        "kwh_period_"
    )
    peak_periods = pd.json_normalize(summary_df["peak_kw_by_period"]).add_prefix(
        "peak_kw_period_"
    )
...
def calculate_utility_costs_for_app(
    tariff_data: Dict, load_profile_path: str, default_voltage: float = 480.0
) -> pd.DataFrame:
    ...
    try:
        ...
        simplified_results["total_energy_cost"] = (
            simplified_results["energy_charge"]
            + simplified_results["energy_adjustment"]
        )
        simplified_results["total_demand_cost"] = (
            simplified_results["demand_charge"]
            + simplified_results["demand_adjustment"]
            + simplified_results["flat_demand_charge"]
            + simplified_results["flat_demand_adjustment"]
        )
...
def main():
    ...
    try:
        ...
        seasonal_summary = (
            df.groupby("season")
            .agg({"total_kwh": "sum", "peak_kw": "max", "total_charge": "sum"})
            .round(2)
        )
