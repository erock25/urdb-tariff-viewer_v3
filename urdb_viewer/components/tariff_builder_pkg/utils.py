        ...
        "items": [
            {
                ...
                "energyratestructure": [[{"unit": "kWh", "rate": 0.0, "adj": 0.0}]],
...
def validate_tariff(tariff_data: Dict) -> Tuple[bool, List[str]]:
    ...
    if not tariff_data.get("energyratestructure"):
        ...
        has_nonzero = any(
            rate[0].get("rate", 0) != 0 for rate in tariff_data["energyratestructure"]
        )
        if not has_nonzero:
...
def show_section_validation(section: str, data: Dict) -> None:
    ...
    if section == "basic_info":
        ...
        has_rates = any(
            rate[0].get("rate", 0) > 0 for rate in data.get("energyratestructure", [])
        )
        if not has_rates:
...
def save_tariff(data: Dict, filename: str) -> None:
    ...
    try:
        ...
        st.info(
            "🔄 Refresh the page or reselect from the sidebar to view your new tariff."
        )
