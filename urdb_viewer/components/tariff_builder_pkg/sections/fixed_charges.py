...
def _render_seasonal_flat_demand(data: Dict) -> None:
    ...
    if len(data["flatdemandstructure"]) != num_seasons:
        data["flatdemandstructure"] = [
            [{"rate": 0.0, "adj": 0.0}] for _ in range(num_seasons)
        ]
    ...
    for month_idx, month in enumerate(MONTHS):
        with cols[month_idx % 4]:
            current_idx = (
                data["flatdemandmonths"][month_idx]
                if month_idx < len(data["flatdemandmonths"])
                else 0
            )
            season = st.selectbox(
                month,
...
def render_fixed_charges_section() -> None:
    ...
    with col2:
        charge_units = st.selectbox(
            ...
            index=["$/month", "$/day", "$/year"].index(
                data.get("fixedchargeunits", "$/month")
            ),
