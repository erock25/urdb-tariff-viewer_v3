...
def get_active_energy_periods_for_month(
    tariff_data: Dict[str, Any], month: int
) -> Set[int]:
    """
...
def get_active_demand_periods_for_month(
    tariff_data: Dict[str, Any], month: int
) -> Set[int]:
    """
...
def calculate_annual_period_hour_percentages(
    ...
    for period, hours in period_hours.items():
        period_percentages[period] = (
            (hours / total_hours * 100) if total_hours > 0 else 0
        )
